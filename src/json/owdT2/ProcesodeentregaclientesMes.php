...
CASE
    ...
    WHEN s.respuestas = 'Lo hizo en el tiempo planificado' THEN 1
    WHEN s.respuestas = 'Supero el tiempo planificado' THEN 0
    WHEN s.respuestas = 'Atendió en menor tiempo' THEN 1
    WHEN s.respuestas = 'Nada satisfecho' THEN 0
    WHEN s.respuestas = 'Insatisfecho' THEN 0
    WHEN s.respuestas = 'Indiferente' THEN 0
    WHEN s.respuestas = 'Satisfecho' THEN 1
    WHEN s.respuestas = 'Muy Satisfecho' THEN 1
...
WHERE id_tema=698827560
AND id_grupo_preguntas <> 38 AND respuesta_user <> ''  /* AND  grupo = '6083868d58c43' */
$WhereAnd
) AS s
) AS b 
GROUP BY  
    ...
    z.preguntas";
        ...
        include '../../bd/banco.php';
            ...
            if (!$select){
                ...
                $arreglo['ProcesoEntregaClientesMes'] = $select->fetchAll(PDO::FETCH_ASSOC);
            }
            if(!empty($arreglo)){
