...
FROM(
SELECT 
s.tema,s.preguntas,s.id_preguntas,s.respuestas,
CASE
    WHEN s.respuestas = 'Si' THEN 1
    WHEN s.respuestas = 'No' THEN 0
    WHEN s.respuestas = 'Faltantes de producto' THEN 0
    WHEN s.respuestas = 'Mala presentación producto' THEN 0
    WHEN s.respuestas = 'Rotura de producto' THEN 0
    WHEN s.respuestas = 'Mala presentación de la tripulación' THEN 0
    WHEN s.respuestas = 'No programación PFN' THEN 0
    WHEN s.respuestas = 'No hubo reclamos' THEN 1
    ELSE 0
...
WHERE id_tema=803151540  
AND id_grupo_preguntas <> 48 AND respuesta_user <> ''
$WhereAnd 
) AS s 
) AS b 
GROUP BY  
    ...
    z.preguntas";
        ...
        include '../../bd/banco.php';
            ...
            if (!$select){
                ...
                $arreglo['VisitaClientesMotivoMes'] = $select->fetchAll(PDO::FETCH_ASSOC);
            }
            if(!empty($arreglo)){
