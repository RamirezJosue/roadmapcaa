<?php
    ...
    CASE
        WHEN b.id_preguntas = '162' THEN ((SUM(b.respuesta_valor) / SUM(b.contador))-1)*-1
        WHEN b.id_preguntas = '164' THEN ((SUM(b.respuesta_valor) / SUM(b.contador))-1)*-1
        WHEN b.id_preguntas = '175' THEN ((SUM(b.respuesta_valor) / SUM(b.contador))-1)*-1
    ...
    WHERE id_tema=903786648
    AND id_grupo_preguntas <> 42  
    AND `respuesta_user` <> ''             
    $WhereAnd 
    ) AS s 
    ) AS b 
    ...
    z.preguntas";
        ...
        include '../../bd/banco.php';
            ...
            if (!$select){
                ...
                $arreglo['LLegadaCamionesMes'] = $select->fetchAll(PDO::FETCH_ASSOC);
            }
            if(!empty($arreglo)){
