...
switch ($op):
    ...
    case "enviarrsdb":
	    ...
	    if($crud->contardbuser('Fecha','t77_rs','Fecha = "'.$fecharsverifi.'" AND centro="'.$idcentro.'"')>0){
		   ...
		   <?php
         header('Refresh: 3; URL=http://www.bk77.co/ls');		   
		} else {
	      $db=Db::conectar();
    ...
    default:	
?>
<p class="h3"> Lista Segura : <?php echo $idcentro; ?></p>
...
<form action="ls" method="post" enctype="multipart/form-data">
   ...
   <button class="btn btn-danger btn-sm" type="button" onclick="location.href='ls?op=borrarbdrs';" <?php echo $disablegrabar; ?> >Borrar datos cargados</button>
   </div>
</form>
<?php	
